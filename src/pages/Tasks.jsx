...
import { Badge, Box, Button, Flex, Heading, Select, Table, Thead, Tbody, Tr, Th, Td, TableContainer } from '@chakra-ui/react';
...
const handleOrderBy = (value) => {
  ...
  const newDirection = currentOrderBy === value && currentDirection === 'asc' ? 'desc' : 'asc';

  searchParams.set('orderBy', value);
  searchParams.set('direction', newDirection);
  setSearchParams(searchParams);
  setSortDirection(newDirection); // Обновляем состояние для UI
};
...
<Heading as="h1" fontSize="3xl" fontWeight="semibold" textAlign="center" my="7">Tasks to do</Heading>
...
<Tr>
<Th>
<Flex onClick={() => handleOrderBy('name')} cursor="pointer">
Task
{searchParams.get('orderBy') === 'name' && (
  sortDirection === 'asc' ? <BsArrowUp /> : <BsArrowUp style={{ transform: 'rotate(180deg)' }} />
)}
</Flex>
</Th>
<Th>
<Flex onClick={() => handleOrderBy('priority')} cursor="pointer">
Priority
...
<Td>
<Badge colorScheme={task.priority === 'urgent' ? 'red' : 'gray'}>{task.priority}</Badge>
</Td>
<Td>
<Badge colorScheme={task.status === 'open' ? 'orange' : 'green'}>{task.status}</Badge>
</Td>
